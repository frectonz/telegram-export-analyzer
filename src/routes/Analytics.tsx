  ...
  return (
    <main className="container mx-auto p-2">
      ...
      <section className="py-8">
        ...
        />
        <section>
          <MembersTable members={members} totalMessages={totalMessages} />
        </section>
      </section>
    </main>
  );
    ...
    <div className="stats stats-vertical lg:stats-horizontal shadow-xl w-full mb-8">
      {stats.map((stat, i) => (
        <div className="stat" key={i}>
          <div className={`stat-figure ${stat.className}`}>{stat.icon}</div>
...
function MembersTable({
  ...
  return (
    <div className="card bg-base-100 shadow-xl my-5 p-4 overflow-x-scroll">
        ...
        <tbody>
          {members.map((member, idx) => (
            <tr key={idx}>
              <th>{idx + 1}</th>
              <td>{member.name}</td>
              <td>{formatNumber(member.messages)}</td>
              <td>
                <progress
